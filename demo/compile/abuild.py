...
CONFIGS = {
    "kconfig": ("KConfigSDK", "KConfigSDK", "KConfigSDKConfig.cmake"),
...
def usage(exit_code: int = 1) -> None:
    ...
    print("  --kconfig-sdk <dir>   required KConfigSDK prefix", file=sys.stderr)
...
def resolve_vcpkg_context(kconfig_sdk_prefix: str, repo_root: str) -> tuple[str, str]:
    candidate_build_dirs: list[str] = []
    if os.path.basename(kconfig_sdk_prefix.rstrip("/\\")) == "sdk":
        candidate_build_dirs.append(os.path.abspath(os.path.dirname(kconfig_sdk_prefix)))
    candidate_build_dirs.append(os.path.abspath(os.path.join(kconfig_sdk_prefix, os.pardir)))
    ...
    print(
        "Error: could not infer vcpkg installed tree/triplet from --kconfig-sdk.\n"
        f"KConfig SDK:\n  {kconfig_sdk_prefix}",
        file=sys.stderr,
    )
    raise SystemExit(2)
...
def main() -> int:
    ...
    while i < len(args):
        ...
        if arg in ("-h", "--help"):
            ...
            build_dir = args[i]
        elif arg == "--kconfig-sdk":
            i += 1
            if i >= len(args):
                fail("missing value for --kconfig-sdk")
            kconfig_sdk_arg = args[i]
        elif arg == "--ktrace-sdk":
            i += 1
            if i >= len(args):
    ...
    if not kconfig_sdk_arg:
        fail("--kconfig-sdk <path> is required")
    ...
    kconfig_sdk_prefix = resolve_prefix(kconfig_sdk_arg, repo_root)
    ktrace_sdk_prefix = resolve_prefix(ktrace_sdk_arg, repo_root)

    validate_sdk_prefix(kconfig_sdk_prefix, "kconfig")
    validate_sdk_prefix(ktrace_sdk_prefix, "ktrace")

    vcpkg_installed_dir, vcpkg_triplet = resolve_vcpkg_context(kconfig_sdk_prefix, repo_root)
    ...
    prefix_path = ";".join([kconfig_sdk_prefix, ktrace_sdk_prefix, vcpkg_prefix])
    ...
    cmake_args = [
        ...
        f"-DKConfigSDK_DIR={os.path.join(kconfig_sdk_prefix, 'lib', 'cmake', 'KConfigSDK')}",
