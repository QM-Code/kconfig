...
std::vector<karma::cli::CliRegisteredOption> BuildGameCliOptions(CLIOptions& opts) {
    std::vector<karma::cli::CliRegisteredOption> options{};
    options.push_back(karma::cli::DefineStringOption(
        "-n",
        "--name",
        "<name>",
    ...
    options.push_back(karma::cli::DefineFlagOption(
        "",
        "--dev-quick-start",
...
void PrintHelp(const std::string& app_name,
    ...
    std::cout
        ...
        << "\n"
        << "Options:\n";
    karma::cli::AppendCommonCliHelp(std::cout, true);
    karma::cli::AppendCoreBackendCliHelp(std::cout);
    karma::cli::AppendClientBackendCliHelp(std::cout);
...
CLIOptions ParseCLIOptions(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        const auto render_result =
            karma::cli::ConsumeRenderBackendCliOption(arg, i, argc, argv, opts.backend_render, opts.backend_render_explicit);
        if (render_result.consumed) {
            if (!render_result.error.empty()) {
                Fail(render_result.error, opts.app_name, game_options);
            }
            continue;
        }

        const auto ui_result =
            karma::cli::ConsumeUiBackendCliOption(arg, i, argc, argv, opts.backend_ui, opts.backend_ui_explicit);
        if (ui_result.consumed) {
            if (!ui_result.error.empty()) {
                Fail(ui_result.error, opts.app_name, game_options);
            }
            continue;
        }
        ...
        const auto platform_result =
            karma::cli::ConsumePlatformBackendCliOption(arg,
                                                        i,
                                                        argc,
                                                        argv,
        ...
        if (platform_result.consumed) {
            if (!platform_result.error.empty()) {
                Fail(platform_result.error, opts.app_name, game_options);
            }
            continue;
        }
