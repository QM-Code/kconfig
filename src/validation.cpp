...
using kconfig::common::config::RequiredKey;
using kconfig::common::config::RequiredType;

bool IsBoolLike(const kconfig::common::serialization::Value& value) {
...
bool IsUInt16Like(const kconfig::common::serialization::Value& value) {
...
bool IsFloatLike(const kconfig::common::serialization::Value& value) {
    return value.is_number_float() || value.is_number_integer();
}

bool IsStringLike(const kconfig::common::serialization::Value& value) {
    return value.is_string();
}

bool IsValidType(const kconfig::common::serialization::Value& value, RequiredType type) {
    switch (type) {
...
std::vector<RequiredKey> ClientKeys() {
    ...
    keys.reserve(21);
    AppendKeys(keys, {
        {"client.Language", RequiredType::String},
