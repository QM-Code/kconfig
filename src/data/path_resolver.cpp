...
void TraceMountTable(const std::optional<ContentMount> &filesystemMount,
                     ...
                     const char *reason) {
    if (!spdlog::default_logger() || !spdlog::default_logger()->should_log(spdlog::level::debug)) {
        return;
    }

    KTRACE("content", "ContentMounts: {} filesystem={} packages={}",
                reason,
                filesystemMount.has_value() ? 1 : 0,
                packageMounts.size());
    if (filesystemMount.has_value()) {
        KTRACE("content", "ContentMounts: fs id='{}' source='{}' mount='{}'",
                    filesystemMount->id,
    ...
    for (size_t index = 0; index < packageMounts.size(); ++index) {
        ...
        KTRACE("content", "ContentMounts: package[{}] id='{}' source='{}' mount='{}'",
                    index,
                    mount.id,
                    mount.source.string(),
...
std::filesystem::path DetectDefaultUserConfigDirectory() {
    ...
    KTRACE("cli", "detecting default config root (app='{}')", appName);

#if defined(_WIN32)
    if (const char *appData = std::getenv("APPDATA"); appData && *appData) {
        base = appData;
        KTRACE("cli", "base from APPDATA='{}'",
                    std::filesystem::path(appData).string());
        ...
        KTRACE("cli", "base from USERPROFILE='{}' -> '{}'",
                    std::filesystem::path(userProfile).string(),
                    base.string());
    } else {
        KTRACE("cli", "APPDATA and USERPROFILE are unset/empty");
    }
#elif defined(__APPLE__)
    if (const char *home = std::getenv("HOME"); home && *home) {
        base = std::filesystem::path(home) / "Library" / "Application Support";
        KTRACE("cli", "base from HOME='{}' -> '{}'",
                    std::filesystem::path(home).string(),
                    base.string());
    } else {
        KTRACE("cli", "HOME is unset/empty");
    }
#else
    if (const char *xdg = std::getenv("XDG_CONFIG_HOME"); xdg && *xdg) {
        base = xdg;
        KTRACE("cli", "base from XDG_CONFIG_HOME='{}'",
                    std::filesystem::path(xdg).string());
        ...
        KTRACE("cli", "XDG_CONFIG_HOME and HOME are unset/empty");
    }
#endif

    if (base.empty()) {
        KTRACE("cli", "failed to determine base user config directory");
...
std::filesystem::path UserConfigDirectory() {
    {
        ...
        if (g_userConfigRootOverride.has_value()) {
            KTRACE("cli", "using override directory '{}'",
                        g_userConfigRootOverride->string());
            return *g_userConfigRootOverride;
        }
    }
    static const std::filesystem::path default_dir = DetectDefaultUserConfigDirectory();
    KTRACE("cli", "using default directory '{}'", default_dir.string());
    return default_dir;
}
...
std::filesystem::path EnsureUserConfigFile(const std::string &fileName) {
    ...
    std::string error;
    if (!path_utils::EnsureJsonObjectFile(filePath, &error)) {
...
std::optional<kconfig::json::Value> LoadJsonFile(const std::filesystem::path &path,
    ...
    if (!result.json.has_value()) {
        switch (result.error) {
        case path_utils::JsonReadError::NotFound:
            spdlog::log(missingLevel, "data_path_resolver: {} not found: {}", label, path.string());
            break;
        case path_utils::JsonReadError::OpenFailed:
            spdlog::error("data_path_resolver: Failed to open {}: {}", label, path.string());
            break;
        case path_utils::JsonReadError::ParseFailed:
            spdlog::error("data_path_resolver: Failed to parse {}: {}", label, result.message);
            break;
        case path_utils::JsonReadError::None:
            break;
        }
        return std::nullopt;
    }
...
void CollectAssetEntries(const kconfig::json::Value &node,
    ...
    path_utils::CollectAssetEntries(node, baseDir, assetMap, prefix);
}
