...
std::unordered_map<std::string, std::string> loadLanguageStrings(std::string_view configNamespace,
    ...
    try {
        ...
        if (!root.is_object()) {
            spdlog::log(missingLevel,
                        "i18n: language asset for '{}' is not a JSON object.",
                        std::string(language));
            return result;
        }
        ...
        spdlog::log(missingLevel,
                    "i18n: failed to load language '{}' from namespace '{}': {}",
                    std::string(language),
...
void LoadLanguage(std::string_view configNamespace, std::string_view language) {
    ...
    state.fallbackStrings = loadLanguageStrings(configNamespace, fallbackLanguage, spdlog::level::err);
    state.selectedStrings.clear();
    ...
    if (selectedLanguage != fallbackLanguage) {
        state.selectedStrings = loadLanguageStrings(configNamespace, selectedLanguage, spdlog::level::warn);
        if (state.selectedStrings.empty()) {
            spdlog::warn("i18n: falling back to '{}'; language '{}' not found or empty.",
                         fallbackLanguage,
                         selectedLanguage);
            selectedLanguage = fallbackLanguage;
        }
