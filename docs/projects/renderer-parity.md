...
- R24 residual risk: trust-root policy is currently bounded to in-engine allowlisted root IDs/secrets for deterministic contract behavior; production trust-root rotation/distribution lifecycle is not implemented yet.
- R24 explicit deferral: cryptographic asymmetric signature verification and external trust-store lifecycle/rotation tooling remain deferred.
