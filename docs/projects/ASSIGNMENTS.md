...
| `physics-refactor.md` | `specialist-physics-refactor` | `in progress (Phase 4h implemented: runtime motion-lock mutation parity landed; full validation blocked by unrelated app compile failures in current workspace state)` | `KARMA intake`: resolve/route around unrelated app compile blockers, rerun Phase 4h validation in `build-a3`, then continue bounded Phase 4 runtime-mutation parity follow-up work. | `2026-02-18` |
