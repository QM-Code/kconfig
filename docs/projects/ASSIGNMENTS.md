...
| `karma-shadow-intake-p0.md` | `overseer` | `priority/in progress (KS0 + KS1 + KS2.1 directional GPU shadow hardening + KS2.2 local-light source contract/shading bridge + KS2.3 bounded point-shadow generation/sampling + KS3 dirty-face invalidation/budget scheduling + KS4.1 budget/map-size sweep + defaults lock + KS4.2 closeout run captured)` | `KARMA intake`: collect operator visual verdict for closeout run `/tmp/point-shadow-visual-closeout-20260216T142607Z` (locked defaults: `pointFacesPerFrameBudget=2`, `pointMapSize=1024`, `pointMaxShadowLights=2`). | `2026-02-16` |
