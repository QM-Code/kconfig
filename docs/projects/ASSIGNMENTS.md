...
| `engine-game-boundary-hygiene.md` | `specialist-engine-boundary-e2` | `in progress (E0.6/E1/E2 landed on build-a1; manifest/cache extraction moved to engine modules with thin wrappers)` | `shared unblocker`: execute E3 delta/package-apply extraction into `common/content` while preserving world-package safety behavior. | `2026-02-17` |
...
| `renderer-backend-file-split.md` | `specialist-renderer-file-split` | `in progress (Phase 4 shadow extraction landed and validated on build-a6)` | `shared unblocker`: execute Phase 5 move-only render extraction by splitting render-layer orchestration/draw loops into `render.cpp` for BGFX and Diligent. | `2026-02-17` |
