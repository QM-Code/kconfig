...
| `physics-refactor.md` | `specialist-physics-refactor` | `in progress (Phase 4e landed: controller runtime velocity now preserves vertical motion, one-shot jump intent is consumed deterministically, and disabled/incompatible controller paths retain rigidbody ownership)` | `KARMA intake`: execute a bounded Phase 4 follow-up to deepen controller-runtime parity beyond velocity/jump mutation handling without introducing backend-native controller objects. | `2026-02-18` |
