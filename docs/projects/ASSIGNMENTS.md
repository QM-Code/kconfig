...
| `physics-refactor.md` | `specialist-physics-refactor` | `in progress (Phase 4g landed: runtime linear/angular damping mutation parity wired across scene contracts, substrate/backends, and ECS sync)` | `KARMA intake`: execute a bounded Phase 4 follow-up for remaining engine-generic runtime mutation parity gaps without introducing backend-native controller objects. | `2026-02-18` |
