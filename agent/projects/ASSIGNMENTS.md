...
| `multiplatform.md` | `overseer` | `in progress (MP5 CI scaffolding landed; first runner evidence pending)` | `CI bring-up`: run new desktop/mobile packaging workflows on GitHub runners, then promote stable jobs to required checks. | `2026-02-21` |
...
| `sdk.md` | `overseer` | `in progress (queued; waiting for multiplatform MP0-SDK1 integration settle)` | `shared unblocker`: dispatch no-compat naming migration slice (`KarmaEngine`/`engine_*` -> `KarmaSDK`/`core|client`) after MP0-SDK1 merge confirmation. | `2026-02-21` |
