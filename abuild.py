...
def usage(exit_code: int = 1) -> None:
    ...
    print(
        "  -k, --kconfig-sdk  core: SDK install prefix override (default: <build-dir>/sdk); "
...
def validate_sdk_prefix(prefix: str) -> None:
    ...
    config_path = os.path.join(prefix, "lib", "cmake", "KConfigSDK", "KConfigSDKConfig.cmake")
...
def resolve_local_ktrace_sdk(repo_root: str) -> tuple[str, str]:
    sdk_prefix = os.path.abspath(
        os.path.join(repo_root, os.pardir, "m-ktrace", "build", "0.1", "sdk")
    )
    if not os.path.isdir(sdk_prefix):
        print(
            "Error: local KTrace SDK directory is missing.\n"
            f"Expected:\n  {sdk_prefix}\n"
            "Build/install m-ktrace SDK first.",
            file=sys.stderr,
        )
        raise SystemExit(2)

    cmake_dir = os.path.join(sdk_prefix, "lib", "cmake", "KTraceSDK")
    config_file = os.path.join(cmake_dir, "KTraceSDKConfig.cmake")
    if not os.path.isfile(config_file):
        print(
            "Error: local KTrace SDK package config is missing.\n"
            f"Expected:\n  {config_file}\n"
            "Build/install m-ktrace SDK first.",
            file=sys.stderr,
        )
...
def main() -> int:
    ...
    while i < len(args):
        ...
        if arg in ("-h", "--help"):
            ...
            build_dir = args[i]
        elif arg in ("-k", "--install-sdk", "--sdk", "--kconfig-sdk"):
            i += 1
            if i >= len(args):
    ...
    ktrace_sdk_prefix, ktrace_sdk_cmake_dir = resolve_local_ktrace_sdk(repo_root)
    build_mode, demo_name = classify_build_dir(build_dir)
    source_dir = repo_root
    cmake_args = ["-DCMAKE_BUILD_TYPE=Release"]
    cmake_prefix_entries = [ktrace_sdk_prefix]
    cmake_args.append(f"-DKTraceSDK_DIR={ktrace_sdk_cmake_dir}")
    demo_sdk_prefix = ""
    ...
    if build_mode == "core":
        ...
        if not sdk_prefix_arg.strip():
            fail("-k/--kconfig-sdk <sdk-prefix> is required for demo build directories")
        ...
        cmake_prefix_entries = [demo_sdk_prefix, ktrace_sdk_prefix, demo_vcpkg_prefix]
        cmake_args.extend(
            [
                f"-DVCPKG_TARGET_TRIPLET={demo_vcpkg_triplet}",
                f"-DVCPKG_INSTALLED_DIR={demo_vcpkg_installed_dir}",
                f"-DKConfigSDK_DIR={os.path.join(demo_sdk_prefix, 'lib', 'cmake', 'KConfigSDK')}",
            ]
        )
        print(
            "Demo build -> "
            f"dir={build_dir} | demo={demo_name} | sdk={demo_sdk_prefix} | "
            f"ktrace_sdk={ktrace_sdk_prefix} | triplet={demo_vcpkg_triplet}",
            flush=True,
        )
    ...
    cmake_args.extend(
        [
            f"-DCMAKE_PREFIX_PATH={';'.join(cmake_prefix_entries)}",
